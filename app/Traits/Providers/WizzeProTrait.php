...
trait WizzeProTrait
{
    ...
    public static function GameLaunchWizzePro($provider_code, $game_code, $lang, $userId)
    {
        if (self::getCredentialsWizzePro()) {
            ...
            $validGames = [
                ...
                '101' => "rise-apollo",
                '87' => "treasures-aztec",
                '1727711' => "three-cz-pigs",
                '67' => "shaolin-soccer",
            ];
    
            if (array_key_exists($game_code, $validGames)) {
                $wallet = Wallet::where('user_id', $userId)->where('active', 1)->first();
    
                error_log($game_code);
    
                $gamename = $validGames[$game_code];
    ...
    private static function ProcessPlayWizzePro($data, $userId, $type)
    {
        ...
        if(!empty($wallet)) {
            ...
            $isDemoAgent = $user->is_demo_agent;
            ...
            if(!empty($transaction)) {
                ...
                if (!empty($lastTransaction)) {
                    ...
                    $lastTransaction->update([
                        ...
                        'type' => $resultType,
                        'is_demo_agent' => $isDemoAgent // Atualiza o campo is_demo_agent
                    ]);
                ...
                if($transaction) {
                    ...
                    Order::where('transaction_id', $data[$type]['txn_id'])
                        ...
                        ->update([
                            ...
                            'is_demo_agent' => $isDemoAgent // Atualiza o campo is_demo_agent// Diferença entre o valor ganho e a aposta
                        ]);
